...
public class LineView extends View {
    ...
    private int topLineLength = dip2px(getContext(), 12);;

    private int sideLineLength = dip2px(getContext(),45)/3*2;
    private int backgroundGridWidth = dip2px(getContext(),45);

    //Constants
    private final int popupTopPadding = dip2px(getContext(),2);
    private final int popupBottomMargin = dip2px(getContext(),5);
    private final int bottomTextTopMargin = sp2px(getContext(),5);
    private final int bottomLineLength = sp2px(getContext(), 22);
    private final int DOT_INNER_CIR_RADIUS = dip2px(getContext(), 2);
    private final int DOT_OUTER_CIR_RADIUS = dip2px(getContext(),5);
    private final int MIN_TOP_LINE_LENGTH = dip2px(getContext(),12);
    ...
    private int[] popupColorArray = {R.drawable.autoonn,R.drawable.autooff,R.drawable.btcharton};
   ...
   // }
    ...
    public LineView(Context context, MainActivity activity,Utilities utl){
        super(context);
        ...
        popupTextPaint.setTextSize(sp2px(getContext(), 13));
        popupTextPaint.setStrokeWidth(5);
        popupTextPaint.setTextAlign(Paint.Align.CENTER);

        bottomTextPaint.setAntiAlias(true);
        bottomTextPaint.setTextSize(sp2px(getContext(),12));
...
// But this code not so good.
    ...
    private void drawPopup(Canvas canvas,String num, Point point,int PopupColor){
        ...
        int sidePadding = dip2px(getContext(),singularNum? 8:5);
        int x = point.x;
        int y = point.y- dip2px(getContext(),5);
...
//        mViewHeight = MeasureSpec.getSize(measureSpec);
    ...
    class Dot{
        ...
        int targetY;
        int linenumber;
        int velocity = dip2px(getContext(),18);
